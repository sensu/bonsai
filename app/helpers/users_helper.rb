module UsersHelper
  ...
  def gravatar_for(user, options = {})
    ...
    if hosted || (user.try(:company) && user.company == ENV['HOST_ORGANIZATION'])
      gravatar_url = ActionController::Base.helpers.asset_url("#{ENV['HOST_LOGO']}")
    else
    ...
    image_tag(gravatar_url, style: "height: #{size}px; width: #{size}px", alt: user.name, class: 'gravatar')
  end
