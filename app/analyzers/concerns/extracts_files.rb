...
module ExtractsFiles
  ...
  def extract_readme(files:, path_method:, file_reader:)
    content, path = extract_file(file_path: /\/readme/i, files: files, path_method: path_method, file_reader: file_reader)
    extension = File.extname(path.to_s).to_s.sub(/\A\./, '').presence  # strip off any leading '.'
