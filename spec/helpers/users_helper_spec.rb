...
describe UsersHelper do
  describe '#gravatar_for' do
    it "returns the image tag for the specified user's gravatar image" do
      ...
      expect(gravatar_for(user)).to match(/https\:\/\/s.gravatar.com\/avatar\/fd876f8cd6a58277fc664d47ea10ad19/)
    end

    it "returns the image tag for the specified user's gravtar image with size" do
      user = create(:user, email: 'johndoe@example.com')
      expect(gravatar_for(user, size: 128)).to match(/https\:\/\/s.gravatar.com\/avatar\/fd876f8cd6a58277fc664d47ea10ad19/)
    end
